...
Workbench 执行和结果回传必须以 `source_id + external_id` 形成身份闭环。`cog_case_display_id` 只用于展示、引用、沟通和按 Case No. lookup，不替代执行与回传身份键。

平台可以提供按 COG Case No. 查询单题的 API，例如 `GET /api/public/cog-cases/lookup?cog_case_no=...`。该 lookup 的输入是人类可读的 `cog_case_display_id`，返回 payload 仍必须携带 `source_id + external_id`，消费者必须用这两个字段作为执行和结果回传身份。

...
def test_load_schema_returns_model_capability_v2_schema() -> None:
    schema = load_schema("model_capability_catalog.v2")

    assert schema["properties"]["schema"]["const"] == MODEL_CAPABILITY_CATALOG_SCHEMA_V2
...
def test_load_fixture_returns_valid_model_capability_v2_catalog() -> None:
    payload = load_fixture("minimal.v2")

    catalog = ModelCapabilityCatalogV2.model_validate(payload)
...
def test_list_fixtures_includes_minimal_v2_fixture() -> None:
    assert "minimal.v2" in list_fixtures()


def test_model_capability_resources_are_json_serializable() -> None:
    json.dumps(load_schema("model_capability_catalog.v1"))
    json.dumps(load_schema("model_capability_catalog.v2"))
    json.dumps(load_fixture("minimal.v1"))
    json.dumps(load_fixture("minimal.v2"))
